...
const Input = styled.input`
  ...
  padding-left: 45px; transition: border-color 0.4s;
  &:focus{
    border-color: #007bff;
    outline: none;
  }
...
function Findemail() {
  ...
  const passwordEdit = ()=>{
    ...
    .then(function(){
        setMessage(`귀하의 이메일 주소는 ${resultEmail.replace(/(.{3}).+(@.+)/, "$1*****$2")}로 메일을 발송 하였습니다.`)
        setIsModalOpen(!isModalOpen)
        return
    })
    .catch(error => {
...
// <></>(프래그먼트)에 key값을 입력하려면 <>안에 React.Fragment를 입력하면 가능하다.